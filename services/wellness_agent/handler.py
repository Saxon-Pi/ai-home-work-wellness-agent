...
# services.wellness_agent.agent から wellness_agent を読み込む
...
現在の室内環境を確認し、必要に応じて健康アドバイスを作成して、LINEに送信してください。
天気ツールを使う場合は、現在日時を基準に現在または近い時間帯の天気を取得してください。
""".strip()

    # Agent 実行
    agent_response = wellness_agent(user_request)
