...
def run_athena_query_for_sensor_history(period: str) -> List[Dict[str, Any]]:
    ...
    config_env = require_report_config()
    ...
    FROM "{config_env['catalog']}"."{config_env['database']}"."{config_env['table']}"
    ...
    response = athena.start_query_execution(
        ...
        QueryExecutionContext={
            "Catalog": config_env["catalog"],
            "Database": config_env["database"],
        },
        ResultConfiguration={
            "OutputLocation": config_env["athena_output_location"],
        },
    )
